 ...
 *
 * `@vitest/pretty-format` picks a serializer by probing `$$typeof`, then `nodeType` and `tagName`, then
 * the Immutable.js markers; the matchers probe `asymmetricMatch` and `_isMockFunction`. None belongs to
 * an Obsidian API, and every one is read while RENDERING a value — so throwing there replaces a failing
 * assertion's diff with `Property "$$typeof" is not mocked in TFile`, hiding what actually differed.
 */
