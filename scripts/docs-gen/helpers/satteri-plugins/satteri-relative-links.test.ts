...
describe('satteriRelativeLinks', () => {
  it('rewrites based internal links relative to the current content page', async () => {
    const node = link('/obsidian-test-mocks/api/alpha/Alpha/#member');
  ...
  it('rewrites a self-link to the current directory instead of emitting a 404-ing double slash', async () => {
    ...
    const plugin = satteriRelativeLinks('/obsidian-test-mocks');

    await plugin.link?.(link('/obsidian-test-mocks/api/path/join/'), context.value);
    await plugin.link?.(link('/obsidian-test-mocks/api/path/join/#overloads'), context.value);
  ...
  it('leaves an internal link unchanged when the document has no file URL', async () => {
    ...
    await visit(link('/obsidian-test-mocks/api/alpha/Alpha/'), context);
  ...
  it('resolves against the current document rather than the one memoized from the previous file', async () => {
    ...
    await plugin.link?.(link('/obsidian-test-mocks/api/alpha/Alpha/'), first.value);
    await plugin.link?.(link('/obsidian-test-mocks/api/alpha/Alpha/'), second.value);
...
async function visit(node: Link, context: MockContext): Promise<void> {
  await satteriRelativeLinks('/obsidian-test-mocks').link?.(node, context.value);
}
