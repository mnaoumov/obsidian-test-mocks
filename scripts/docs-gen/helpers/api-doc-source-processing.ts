 ...
 *
 * `internal` is the package's private implementation detail (L1 in `AGENTS.md` — nothing there is
 * exported), `jest` holds only the Jest-runner smoke test, and `test-helpers` is a one-line re-export
 * that has no published subpath.
 */
const EXCLUDED_DIR_SEGMENTS = new Set(['@types', 'internal', 'jest', 'test-helpers']);
